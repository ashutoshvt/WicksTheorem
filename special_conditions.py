# Algorithm:
# This file has the sub routine for a problem of doing contraction two operators at a time.
# When there is a commutator of operators, there can be a reduced factor of the term.
# This can happen when there are two operators of the same kind and equivalent, but the
# commutator type contraction structure in this program
# does not count a part of the possible contractions.
# The solution is based on two cases:
# 1. Case1: when one of the two commutators do not have a connection with one operator
# with order (commutator number) less than the
# lower of the two equivalent operators.
# 2. Case 2: when there are operators in the middle of the two equivalent
# operators, and any of those operators have a connection to an
# operator with lower order which is not among the equivalent operators.
# If any of the case is true, multiply the term with two corresponding to the missing part.

def create_map(term, equivop):
    # for op in term.map_org:
    # at least 2 equivalent operators present with one of them as the first contraction.
    map_out = []
...
def create_map2(term, equivop):
    ...
    for op in term.large_op_list:
        if len(op.name) > 2:
            if posop1 < int(op.name[2]) < posop2:
                oplistmiddle.append(op.name)
            elif posop1 > int(op.name[2]) > posop2:
                oplistmiddle.append(op.name)
    output = []
...
def non_equivop(term, map_out, equivop):
    pos1 = 10000  # stores position of the first occurring equivop
    found_pos = 0
    ...
    for op in term.large_op_list:
        if equivop in op.name:
            ...
            if len(op.name) > 2:
                ...
                pos1 = 0
                # print(op.name[2],pos1)
    # if each equiv op is connected to a arm which is created before pos1,
    # it is equiv. is one of the arms in one is created after the first pos, it is non-eq
    for opeq in map_out:
        inner_comm = 0
        for op1 in opeq:
            if len(op1) > 2:  # case when the operator can be in outer comm
                pos2 = int(op1[2])
                if pos2 < pos1:
                    inner_comm = 1
            elif op1[0] == 'V':
...
def non_equivop2(term, map_out2, equivop, opmiddle):
    ...
    # case2: if the there is one connection in the middle operator which
    # is not an equivalent operator and has comm position less than middle operator, it returns 0
    # print('inside two')
...
def startequiv_cond(list_terms):
    ...
    for term in list_terms:
        ...
        if t1 > 1:
            ...
            if case1 == 1.0 or case2 == 1.0:
                # print('special condition involved')
                term.fac = term.fac*2.0
        if t2 > 1:
            equivop = 'T2'
            map_out = create_map(term, equivop)
