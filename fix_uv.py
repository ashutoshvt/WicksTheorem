# This file is the printing part of the whole program. Everything is being printed here
...
def fix_con(op_no, cnt, lim_cnt, t_list, matched, contracted, contracted_l, contracted_r, a, i, u, full, f, fptr,
            full_pos, i_c, contr_obj, const_obj):

    if cnt < lim_cnt != 0:
        t_list_tmp = copy.deepcopy(t_list)
        t_list_tmp.popleft()
        op_no = op_no+1  # now the +1th annihilation operator is being seen
        while t_list_tmp:
            flag = 1
            while flag:
                flag = 0
            ...
            fix_con(copy.copy(op_no), cnt+1, lim_cnt, copy.deepcopy(t_list_tmp), matched, contracted,
                    contracted_l, contracted_r, a, i, u, full, f, fptr, full_pos, i_c, contr_obj, const_obj)
            matched.pop()
            matched.pop()
            t_list_tmp[0].popleft()
    ...
    # case when the number of contractions required are done. This we only have to print them
        ...
        # AK: conserving the tensor ordering logic for uncontracted indices
        tensor_order_map = {}
        output1 = []
        new_list1 = []
        ...
        try:
            ...
            # formed cummulants being appended in new_list------------------------
            # const_of_cummulant=func.cummulant(contracted, full_formed, new_list)
            const_of_cummulant = 0.0

            if const_of_cummulant:
                const_of_expression = const_of_expression * const_of_cummulant
            print("const of expression and cummulant", const_of_expression, const_of_cummulant)
            # the summation thingy
            if func.loop_present(spin_list_upper, spin_list_lower, -1, 0):
                # if not output and const_of_expression!=1.0 and not cummulant_present :
    ...
    # The case when no contractions are made, but cummulants are there/not there
        ...
        if not i_c:  # when the string is 1st part of a commutator
            ...
            sign = -1
        # This is where the cummulants are made through 'cummulant' function and the Latex expression is
        # stored in new_list to be printed later
        # cummulant_present=0
        # const_of_cummulant = func.cummulant(contracted, full_formed, new_list)
        # AK: cummulant not being activated!
        const_of_cummulant = 0.0
        ...
        if output:
            func.write_normal_order(new_list, output)
            # as the name suggest - writes the normal order in output file list
        if parity.parity(full_formed, full_pos):
            sign = sign*(-1)
        if sign == (-1):
            tmp_5 = '$$'+"-"+str(const_of_expression)+''.join(new_list)+"\\\\"+'$$'+'\n'
