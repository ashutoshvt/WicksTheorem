...
class helper_CCSD(object):
    def __init__(self, nmo, nocc, H, MO, rhf_e, freeze_core=False, memory=2):
        """
        ...
        self.nso = self.nmo * 2
        self.MO = np.zeros((self.nso, self.nso, self.nso, self.nso))
            ...
            for q in range(self.nso):
                for r in range(self.nso):
                    for s in range(self.nso):
                        ...
                        R=int(r/2)
                        S=int(s/2)
                        value1 = MO[P][Q][R][S] * int((p%2==r%2)) * int((q%2==s%2))
                        value2 = MO[P][Q][S][R] * int((p%2==s%2)) * int((q%2==r%2))
                        self.MO[p][q][r][s] = value1 - value2
                        
        print("Size of the ERI tensor is %4.2f GB, %d basis functions." % (ERI_Size, self.nmo))
        # Update nocc and nvirt TODO! --> Make sure all the code below works!
        #self.nso = self.nmo * 2
...
if __name__ == "__main__":
    filename = ''
    #if sys.argv[1] == "F12":
    #    filename = 'uccf12.hbar.csv'
    #elif sys.argv[1] == "Regular":
    #    filename = 'uccf12.h.csv'
    ...
    for i in range(nmo*nmo*nmo*nmo):
        ...
        r = int(tmp[2])
        s = int(tmp[3])
        MO[p][q][r][s] = float(tmp[4])

    scf = HelperSCF(nmo, nocc, H, MO, e_nuc, memory=2)
    scf.compute_energy(e_conv=1e-13)
    SCF_E = scf.SCF_E
    print('\nSCF energy:          {}'.format(SCF_E))
    ccsd = helper_CCSD(nmo, nocc, scf.H, scf.MO, SCF_E, memory=2)
    ccsd.compute_energy(e_conv=1e-10)
    CCSDcorr_E = ccsd.ccsd_corr_e
    print('\nCCSD correlation energy:          {}'.format(CCSDcorr_E))
    print('\nTotal energy:          {}'.format(CCSDcorr_E + SCF_E))
    I_file.close()

    # TODO! generate CC_data.txt !!!

    f= open('CC_data.txt','w')
    f.write('{} {}\n'.format(ccsd.nocc, ccsd.nvirt))
    for p in range(ccsd.nso):
        for q in range(ccsd.nso):
            f.write('{}\n'.format(ccsd.F[p][q]))
    
    for p in range(ccsd.nso):
        for q in range(ccsd.nso):
            for r in range(ccsd.nso):
                for s in range(ccsd.nso):
                    f.write('{}\n'.format(ccsd.MO[p][q][r][s]))
    
    for i in range(ccsd.nocc):
        for a in range(ccsd.nvirt):
            f.write('{}\n'.format(ccsd.t1[i][a]))
    
    for i in range(ccsd.nocc):
        for j in range(ccsd.nocc):
            for a in range(ccsd.nvirt):
                for b in range(ccsd.nvirt):
                    f.write('{}\n'.format(ccsd.t2[i][j][a][b]))
    f.close()
