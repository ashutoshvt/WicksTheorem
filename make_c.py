...
fix_temp = fix_uv
func = func_ewt
def make_c(lim_cu, contracted, a, i, u, full, poss, f, fptr, store_for_repeat, full_pos, i_c, menu, contr_obj, const_obj):
    for n in range(2, lim_cu+1, 2):
		...
		if add==0:
			...
			if (flag1 ==0 and p_1==1 and p_2==1) or menu == '1':
			    ...
			    make_c(n, contracted, a, i, copy.deepcopy(u_2), copy.deepcopy(full_2), poss, f, fptr, store_for_repeat, full_pos, i_c, menu, contr_obj, const_obj)#call the function again with smaller u
			    #if not u_2:
            ...
            for lim_cnt in range(0, no+1):
                if lim_cnt>0:
            		...
            		if temp_list:
                	    ...
                	    temp_list[0].popleft()
                	    fix_temp.fix_con(copy.copy(op_no), 1, lim_cnt, copy.deepcopy(temp_list), matched, contracted, contracted_l, contracted_r, a, i, u, full, f, fptr, full_pos, i_c, contr_obj, const_obj)
                	    matched.pop()
                	    matched.pop()
    	        else :
        	    temp_list = deque([])
