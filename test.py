...
def construct_transcorr_H(H_1body, H_2body, info):
    ...
    slice_act = slice(0, nocc+1)
    ncabs = R2_gg_cc.shape[2]
    slice_cabs = slice(nvir, nvir+ncabs)
    slice_vir = slice(0,nvir)
    ...
    #  H1_R1  
    H_1body[:, :] += 1.0 * np.einsum('pX,PX->pP', H1_gc[:, :], R1[:, slice_cabs], optimize=True)
    #  H1_R1D  
    H_1body[:, :] += 1.0 * np.einsum('PX,qX->Pq', R1[:, slice_cabs], H1_gc[:, :], optimize=True)
    #  V2_R1  
    H_2body[:, :, :, :] += 0.5 * np.einsum('pqrX,PX->pqrP', V2_gg_gc[:, :, :, :], R1[:, slice_cabs], optimize=True)
    H_2body[:, :, :, :] += 0.5 * np.einsum('qpsX,PX->pqPs', V2_gg_gc[:, :, :, :], R1[:, slice_cabs], optimize=True)
    #  V2_R1D  
    H_2body[:, :, :, :] += 0.5 * np.einsum('PX,srqX->Pqrs', R1[:, slice_cabs], V2_gg_gc[:, :, :, :], optimize=True)
    H_2body[:, :, :, :] += 0.5 * np.einsum('PX,rspX->Ppsr', R1[:, slice_cabs], V2_gg_gc[:, :, :, :], optimize=True)
    #  F1_R1_R1  
    #  F1_R1_R1D  
    H_1body[:, :] += 0.5 * np.einsum('QY,YX,PX->QP', R1[:, slice_cabs], F1_cc, R1[:, slice_cabs], optimize=True)
    H_1body[:, :] += -0.5 * np.einsum('QY,PY,Pq->Qq', R1[:, slice_cabs], R1[:, slice_cabs], F1_gg[:, :], optimize=True)
    #  F1_R1D_R1  
    H_1body[:, :] += -0.5 * np.einsum('pq,qX,QX->pQ', F1_gg[:, :], R1[:, slice_cabs], R1[:, slice_cabs], optimize=True)
    H_1body[:, :] += 0.5 * np.einsum('PX,XY,QY->PQ', R1[:, slice_cabs], F1_cc, R1[:, slice_cabs], optimize=True)
    #  F1_R1D_R1D  
    #  F1_R1_R2  
    #  F1_R1_R2D  
    H_2body[:, :, slice_v, :] += 0.25 * np.einsum('tuay,yX,PX->tuaP', R2_gg_vc[:, :, :, :], F1_cc, R1[:, slice_cabs], optimize=True)
    H_2body[:, :, :, slice_v] += 0.25 * np.einsum('utbx,xX,PX->tuPb', R2_gg_vc[:, :, :, :], F1_cc, R1[:, slice_cabs], optimize=True)
    H_2body[:, :, slice_v, :] += -0.25 * np.einsum('tuay,Py,Pq->tuaq', R2_gg_vc[:, :, :, :], R1[:, slice_cabs], F1_gg[:, :], optimize=True)
    H_2body[:, :, :, slice_v] += -0.25 * np.einsum('utbx,Px,Pq->tuqb', R2_gg_vc[:, :, :, :], R1[:, slice_cabs], F1_gg[:, :], optimize=True)
    #  F1_R1D_R2  
    H_2body[:, slice_v, :, :] += -0.25 * np.einsum('pq,qX,utbX->pbtu', F1_gg[:, :], R1[:, slice_cabs], R2_gg_vc[:, :, :, :], optimize=True)
    H_2body[:, slice_v, :, :] += -0.25 * np.einsum('pq,qX,tuaX->paut', F1_gg[:, :], R1[:, slice_cabs], R2_gg_vc[:, :, :, :], optimize=True)
    H_2body[:, slice_v, :, :] += 0.25 * np.einsum('PX,Xx,utbx->Pbtu', R1[:, slice_cabs], F1_cc, R2_gg_vc[:, :, :, :], optimize=True)
    H_2body[:, slice_v, :, :] += 0.25 * np.einsum('PX,Xy,tuay->Paut', R1[:, slice_cabs], F1_cc, R2_gg_vc[:, :, :, :], optimize=True)
    #  F1_R1D_R2D  
    #  F1_R2_R1  
    #  F1_R2_R1D  
    H_2body[:, slice_v, :, :] += 0.25 * np.einsum('PX,Xx,utbx->Pbtu', R1[:, slice_cabs], F1_cc, R2_gg_vc[:, :, :, :], optimize=True)
    H_2body[:, :, :, :] += 0.25 * np.einsum('PX,px,tuxX->Pput', R1[:, slice_cabs], F1_gc[:, :], R2_gg_cc, optimize=True)
    H_2body[:, slice_v, :, :] += 0.25 * np.einsum('PX,Xy,tuay->Paut', R1[:, slice_cabs], F1_cc, R2_gg_vc[:, :, :, :], optimize=True)
    H_2body[:, :, :, :] += 0.25 * np.einsum('PX,py,tuXy->Pptu', R1[:, slice_cabs], F1_gc[:, :], R2_gg_cc, optimize=True)
    H_2body[:, slice_v, :, :] += -0.25 * np.einsum('PX,utbX,uq->Pbtq', R1[:, slice_cabs], R2_gg_vc[:, :, :, :], F1_gg[:, :], optimize=True)
    H_2body[:, slice_v, :, :] += -0.25 * np.einsum('PX,tuaX,uq->Paqt', R1[:, slice_cabs], R2_gg_vc[:, :, :, :], F1_gg[:, :], optimize=True)
    H_2body[:, slice_v, :, :] += -0.25 * np.einsum('PX,utbX,tq->Pbqu', R1[:, slice_cabs], R2_gg_vc[:, :, :, :], F1_gg[:, :], optimize=True)
    H_2body[:, slice_v, :, :] += -0.25 * np.einsum('PX,tuaX,tq->Pauq', R1[:, slice_cabs], R2_gg_vc[:, :, :, :], F1_gg[:, :], optimize=True)
    #  F1_R2D_R1  
    H_2body[:, :, slice_v, :] += -0.25 * np.einsum('pq,quay,Py->puaP', F1_gg[:, :], R2_gg_vc[:, :, :, :], R1[:, slice_cabs], optimize=True)
    H_2body[:, :, :, slice_v] += -0.25 * np.einsum('pq,uqbx,Px->puPb', F1_gg[:, :], R2_gg_vc[:, :, :, :], R1[:, slice_cabs], optimize=True)
    H_2body[:, :, slice_v, :] += -0.25 * np.einsum('pq,qtbx,Px->ptbP', F1_gg[:, :], R2_gg_vc[:, :, :, :], R1[:, slice_cabs], optimize=True)
    H_2body[:, :, :, slice_v] += -0.25 * np.einsum('pq,tqay,Py->ptPa', F1_gg[:, :], R2_gg_vc[:, :, :, :], R1[:, slice_cabs], optimize=True)
    H_2body[:, :, slice_v, :] += 0.25 * np.einsum('tuay,yX,PX->tuaP', R2_gg_vc[:, :, :, :], F1_cc, R1[:, slice_cabs], optimize=True)
    H_2body[:, :, :, :] += 0.25 * np.einsum('tuxy,qy,Px->tuPq', R2_gg_cc, F1_gc[:, :], R1[:, slice_cabs], optimize=True)
    H_2body[:, :, :, :] += 0.25 * np.einsum('tuxy,qx,Py->tuqP', R2_gg_cc, F1_gc[:, :], R1[:, slice_cabs], optimize=True)
    H_2body[:, :, :, slice_v] += 0.25 * np.einsum('utbx,xX,PX->tuPb', R2_gg_vc[:, :, :, :], F1_cc, R1[:, slice_cabs], optimize=True)
    #  F1_R2D_R1D  
