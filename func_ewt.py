        ...
        for itema in contracted:
            for itemb in store:
                if len(itema) == len(itemb):
                    flag1 = 0
                    for indx in range(len(itema)):
                        if itema[indx].pos != itemb[indx].pos:
                            flag1 = 1
                # break
                    if flag1 == 0:
                        count_eq = count_eq+1
...
# AK I have removed a redundant argument from the function below!


def arrange(cum_d, cum_n, cum_n_pos):
    for indx in range(len(cum_d)):
        for item in cum_n:
            if cum_d[indx].pair == item.pos:
                ...
                # exchange positions
                swap_p = cum_n_pos[indx]
                pos_item = cum_n_pos.index(item.pos)
                cum_n_pos[indx] = item.pos
...
def write_normal_order(new_list, output):
    ...
    for item in reversed(output):
        # remember here the 1st string comes first as in writing in E has different rules than straight
